    ...
    createNewAcctSubmitListener() {
        ...
        newAcctBtn.addEventListener("click", function () {
            if (userPasswordInput.value !== userPasswordConfirmInput.value) {
                ...
                setTimeout(function () { document.getElementById("alert").innerHTML = `Passwords must match!` }, 180);
                ...
                API.get("users").then(objects => {
                    ...
                    if (emailAuth === true && emailAuth2 === true) {
                        if (userNameInput.value === "" || userEmailInput.value === "" || userPasswordInput.value === "" || userPasswordConfirmInput.value === "" || registerNewUserPhotoURL.value === "") {
                            document.getElementById("alert").innerHTML = "";
                            setTimeout(function () { document.getElementById("alert").innerHTML = `I want all of your info, not just some of it... fill out all of the fields. They're all required and there is nothing you can do about it. Unless you run console.log and find the correct methods to run and hack into the app. In that case there is nothing I can do, and must admit defeat. Your move.` }, 180);
                            return "";
                        } else {
                            let newUserObject = {
                        ...
                        document.getElementById("alert").innerHTML = "";
                        setTimeout(function () { document.getElementById("alert").innerHTML = message }, 180);
                    }
                    return "";
